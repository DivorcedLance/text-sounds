...
def __request(settings):
    ...
    params = __buildRequest(settings)
    url = 'https://api.voicerss.org' if 'ssl' in settings and settings['ssl'] else 'http://api.voicerss.org'
    
    response = requests.post(url, data=params, headers=headers)
    
    if response.status_code != 200:
        result['error'] = response.reason
    elif response.text.find('ERROR') == 0:
